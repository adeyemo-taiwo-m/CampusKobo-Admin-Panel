...
const ContentFormPage = () => {
  ...
  const { data: existingContent, isLoading: contentLoading } = useContentById(
    id || "",
  );
  ...
  const validate = () => {
    ...
    if (!formData.id?.trim()) newErrors.id = "Content ID is required";
      ...
      newErrors.id = "Only lowercase letters, numbers, and hyphens allowed";
  ...
  const handleFileUpload = async (
    ...
    field: "cover_image_url" | "media_url",
  ) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("learning-assets").getPublicUrl(filePath);
  ...
  if (isEditMode && contentLoading) {
    return (
      <div className="h-64 flex items-center justify-center animate-pulse bg-gray-100 rounded-xl" />
    );
  }
      ...
      <PageHeader
        ...
        subtitle={
          isEditMode
            ...
            : "Create a new article, video, or podcast"
        }
      />
          ...
          <div className="card p-6 space-y-8">
            ...
            <div className="space-y-4">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-gray-400">
                  Add 3–5 short bullet points summarizing the content
                </span>
              </div>
            ...
            <Input
              ...
              onChange={(e) =>
                setFormData({
                  ...formData,
                  related_content_ids: e.target.value
                    .split(",")
                    .map((s) => s.trim())
                    .filter(Boolean),
                })
          ...
          <div className="card p-6 space-y-6 sticky top-20">
                  ...
                  <label className="flex flex-col items-center justify-center aspect-video rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 hover:bg-gray-100 hover:border-[#1A9E3F] transition-all cursor-pointer group">
                    {uploadingCover ? (
                      ...
                      <>
                        ...
                        <span className="text-xs font-bold text-gray-400 group-hover:text-[#1A9E3F]">
                          Upload Cover
                        </span>
                      </>
                    )}
                    ...
                    <label className="flex items-center gap-3 p-4 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 hover:bg-gray-100 hover:border-[#1A9E3F] transition-all cursor-pointer group">
                      <div className="p-2 bg-white rounded-lg shadow-sm group-hover:text-[#1A9E3F]">
                        {uploadingMedia ? (
                          <Loader2
                            size={20}
                            className="animate-spin text-[#1A9E3F]"
                          />
                        ) : (
                          <UploadCloud size={20} />
                        )}
            ...
            {isFinance101 && (
              <Input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    episode_number: parseInt(e.target.value) || null,
                  })
                }
            ...
            {formData.is_featured && (
              <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-lg text-amber-700 text-xs font-medium">
                ...
                <p>
                  This will replace the current featured item on the student app
                  home screen.
                </p>
              </div>
            )}
              ...
              <div className="pt-6 border-t border-gray-100">
                <div className="p-4 bg-red-50 rounded-xl border border-red-100">
                  ...
                  <p className="text-[11px] text-red-600 mb-3">
                    Removing this will permanently delete it from the student
                    app.
                  </p>
                  <Button
                    type="button"
